...
{    
    ...
    if(empty($title) || empty($article) || empty($path)) {            
        ...
        //updating the table
        $result = mysqli_query($mysqli, "UPDATE users SET title='$title',article='$article',imagename='$path' WHERE id=$id");
...
<?php
//getting id from url
$id = isset($_GET['id']) ? $_GET['id'] : (isset($_POST['id']) ? $_POST['post'] : 0);
 // print "SELECT title, article FROM users WHERE id=$id";
//selecting data associated with this particular id
$result = mysqli_query($mysqli, "SELECT title, article FROM users WHERE id=$id");
...
<nav class="navbar navbar-expand-lg navbar-light bg-light">
    ...
    <div class="col-12 float-left mt-4">
        <div class="container">
            <form name="form1" method="post" action="edit.php?id=<?$id?>" enctype="multipart/form-data">
    
                <div class="form-group">
                            <input type="text" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" value="<?php echo $title;?>" name="title" required>
                        </div>
                        <div class="form-group">
                            <input type="file" class="form-control " name="uploaded_file" required>
                        </div>
                        <div class="form-group">
                            <textarea name="article" value=""  style="width: 100%;" class="form-control" required><?php echo $article;?></textarea>
                        </div>
